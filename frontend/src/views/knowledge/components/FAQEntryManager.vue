<template>
  <div class="faq-manager">
    <div class="faq-content">
      ...
      <!-- Import progress bar (displayed at top of list page) -->
                ...
                >
                  ...
                  <div class="faq-card-footer">
                    ...
                    <div class="faq-card-status" @click.stop>
                      <!-- Temporarily hidden recommendation switch
                      <t-tooltip
          ...
          <div class="settings-group">
            ...
            <div class="setting-row vertical">
              <div class="setting-info">
                ...
                <p class="desc">{{ $t('knowledgeEditor.faq.tagDesc') || 'Select category for FAQ entry' }}</p>
              </div>
              <div class="setting-control">
                <t-select
...
const tagDropdownOptions = computed(() => [
  { content: t('knowledgeBase.untagged') || 'Untagged', value: '' },
...
const tagSelectOptions = computed(() => [
  { label: t('knowledgeBase.untagged') || 'Untagged', value: '' },
...
const handleFaqMenuAction = (event: Event) => {
  ...
  if (detail.action === 'create') {
    ...
    // Batch operations handled through left menu dropdown
...
const handleEntryStatusChange = async (entry: FAQEntry, value: boolean) => {
  ...
  // Get actual object reference from array to ensure using latest data
  const actualEntry = entries.value[entryIndex]
  const previous = actualEntry.is_enabled
  if (previous === value) {
    return
  }
  // Directly update property, Vue 3 reactive system should detect it
...
const loadEntries = async (append = false) => {
  ...
  try {
    ...
    // Check if need to continue loading to fill visible area
...
// Check if content fills visible area, if not and there's more data, continue loading
const checkAndLoadMore = () => {
  ...
  // If content height is less than container height + 50px buffer, may not have scrollbar or near bottom, need to continue loading
