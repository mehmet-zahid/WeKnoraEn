...
// Float popup state (shared by Web/KB)
...
const intermediateStepsSummary = computed(() => {
  ...
  const parts: string[] = [];
  if (searchCount > 0) {
    parts.push(`Searched knowledge base <strong>${searchCount}</strong> time${searchCount > 1 ? 's' : ''}`);
  }
  if (thinkingCount > 0) {
    parts.push(`Thought <strong>${thinkingCount}</strong> time${thinkingCount > 1 ? 's' : ''}`);
  }
  ...
  if (parts.length === 0) {
    return `<strong>${intermediateStepsCount.value}</strong> intermediate step${intermediateStepsCount.value > 1 ? 's' : ''}`;
  }
  
  // Optimize connecting words to make sentences flow better
  if (parts.length === 1) {
    return parts[0];
  } else if (parts.length === 2) {
    return `${parts[0]}，${parts[1]}`;
  } else {
    // 3 or more: use commas for first few, comma for last
...
// KB citations: hover shows summary in popup; click jumps to KB details
type KbTooltipState = {
  loading: boolean;
  error?: string;
  html?: string;
...
const isBookIcon = (toolName: string): boolean => {
  return false; // No longer using t-icon's book, switched to SVG icon
};
...
.event-item {
  ...
  // First event's connection starts from node
  &:first-child::before {
    top: 14px;
  }
  
  // Last event doesn't show bottom connection
  &.event-last::before {
    bottom: auto;
    height: 22px;
  }
  ...
  &.no-timeline {
    ...
    // Ensure timeline for all event types is hidden (using stronger selector)
    &.event-last::before,
    &.event-last::after,
    &:first-child::before,
    &:first-child::after,
...
.thinking-event {
  ...
  .thinking-phase {
    ...
    &.thinking-last {
      ...
      // When last Thinking is final answer, font should be larger
      .thinking-content {
        font-size: 14px;
      }
    }
