...
watch([() => route.params], (newvalue) => {
    ...
    if (newvalue[0].chatid) {
        ...
        // Reset state when switching conversations (loading history messages should not show loading)
        loading.value = false;
...
const reconstructEventStreamFromSteps = (agentSteps, messageContent, isCompleted = false) => {
    ...
    // Always add answer event if there's content (regardless of agent_steps)
    // This ensures final answer is always rendered
    if (messageContent && messageContent.trim()) {
        ...
        // If message is complete but content is empty (common in Agent mode), add empty answer event to mark completion
        // This ensures isConversationDone returns true, don't show loading-indicator
        events.push({
            type: 'answer',
            content: '',
            done: true
...
const handleStopGeneration = () => {
    ...
    loading.value = false;
    isReplying.value = false;
    // Note: Don't clear currentAssistantMessageId here, need it to call API
    // After API call succeeds, backend stop event will clear it
};
...
const sendMsg = async (value, modelId = '', mentionedItems = []) => {
    ...
    await startStream({ 
        ...
        agent_id: selectedAgentId,  // Pass selected agent ID
