...
// NewInitializationHandler Create initialization handler
...
// KBModelConfigRequest Knowledge base model configuration request (simplified, only model IDs)
...
func (h *InitializationHandler) UpdateKBConfig(c *gin.Context) {
	...
	// Check if Embedding model can be modified
	if kb.EmbeddingModelID != "" && kb.EmbeddingModelID != req.EmbeddingModelID {
		...
		if err == nil && knowledgeList != nil && knowledgeList.Total > 0 {
			...
			c.Error(errors.NewBadRequestError("Knowledge base already has files, cannot modify Embedding model"))
			return
		}
	}

	// Get model details from database and validate
	...
	// Update knowledge base model IDs
	...
	if req.Multimodal.Enabled {
		...
		// Clear storage configuration when multimodal is not enabled
		kb.StorageConfig = types.StorageConfig{}
	}
...
// @Description  Execute complete configuration update by knowledge base ID
// @Tags         Initialization
// @Accept       json
// @Produce      json
// @Param        kbId     path      string  true  "Knowledge base ID"
// @Param        request  body      object  true  "Initialization request"
// @Success      200      {object}  map[string]interface{}  "Initialization successful"
// @Failure      400      {object}  errors.AppError         "Invalid request parameters"
// @Security     Bearer
// @Security     ApiKeyAuth
...
func (h *InitializationHandler) validateMultimodalConfig(ctx context.Context, req *InitializationRequest) error {
	...
	if req.Multimodal.VLM == nil {
		...
		return errors.NewBadRequestError("VLM information must be configured when enabling multimodal")
	}
...
func validateNodeExtractConfig(ctx context.Context, req *InitializationRequest) error {
	...
	if strings.ToLower(os.Getenv("NEO4J_ENABLE")) != "true" {
		...
		return errors.NewBadRequestError("Please correctly configure environment variable NEO4J_ENABLE")
	}
	...
	if len(req.NodeExtract.Nodes) == 0 || len(req.NodeExtract.Relations) == 0 {
		logger.Error(ctx, "Node Extractor configuration incomplete")
		return errors.NewBadRequestError("Please extract entities and relationships first")
	}
	return nil
}
