...
// Check if request is in the list of APIs that do not require authentication
func isNoAuthAPI(path string, method string) bool {
	for api, methods := range noAuthAPI {
		// If ends with *, match by prefix, otherwise match by full path
...
func canAccessTenant(user *types.User, targetTenantID uint64, cfg *config.Config) bool {
	...
	// 4. User has cross-tenant permission, allow access (specific validation done in middleware)
	return true
}
