...
func (s *knowledgeService) CreateKnowledgeFromFile(ctx context.Context,
	...
	if !IsImageType(getFileType(fileName)) {
		...
		case "cos":
			if kb.StorageConfig.SecretID == "" || kb.StorageConfig.SecretKey == "" ||
				...
				return nil, werrors.NewBadRequestError("Uploading image files requires complete object storage configuration, please complete it in system settings")
			}
		case "minio":
		...
		if !kb.VLMConfig.Enabled || kb.VLMConfig.ModelID == "" {
			...
			return nil, werrors.NewBadRequestError("Uploading image files requires VLM model configuration")
		}
	...
	if !isValid {
		...
		return nil, werrors.NewValidationError("Filename contains invalid characters")
	}
...
func (s *knowledgeService) CreateKnowledgeFromManual(ctx context.Context,
	...
	if !ok {
		return nil, werrors.NewValidationError("Title contains invalid characters or exceeds length limit")
	}
...
func (s *knowledgeService) createKnowledgeFromPassageInternal(ctx context.Context,
	...
	// Validate passage content safety
	...
	for i, p := range passage {
		...
		if !isValid {
			...
			return nil, werrors.NewValidationError(fmt.Sprintf("Passage %d contains invalid content", i+1))
		}
	...
	if syncMode {
		...
		taskPayload := types.DocumentProcessPayload{
			...
			EnableMultimodel:         false, // Text paragraphs do not support multimodal
			EnableQuestionGeneration: enableQuestionGeneration,
			QuestionCount:            questionCount,
		}
...
func (s *knowledgeService) processChunks(ctx context.Context,
	...
	// Idempotency handling: clean up old chunks and index data to avoid duplicate data
	...
	if err := s.chunkService.DeleteChunksByKnowledgeID(ctx, knowledge.ID); err != nil {
		...
		// Don't return error, continue processing (may not have old data)
	}
	...
	// ========== DocReader parsing result logs ==========
	logger.Infof(ctx, "[DocReader] ========== Parsing Result Overview ==========")
	...
	logger.Infof(ctx, "[DocReader] ========== Parsing Result Overview End ==========")

	// Create chunk objects from proto chunks
	maxSeq := 0

	// Count image-related sub-chunks for expanding insertChunks capacity
	imageChunkCount := 0
	for _, chunkData := range chunks {
		if len(chunkData.Images) > 0 {
			// Create a Chunk for each image's OCR and Caption separately
	...
	for _, chunkData := range chunks {
		...
		if len(chunkData.Images) > 0 {
			...
			for i, img := range chunkData.Images {
				// Save image information to text Chunk
				...
				// If there is OCR text, create OCR Chunk
				if img.OcrText != "" {
					ocrChunk := &types.Chunk{
						...
						KnowledgeBaseID: knowledge.KnowledgeBaseID,
						Content:         img.OcrText,
						ChunkIndex:      maxSeq + i*100 + 1, // Use non-conflicting index method
						IsEnabled:       true,
						CreatedAt:       time.Now(),
						UpdatedAt:       time.Now(),
				...
				// If there is image description, create Caption Chunk
				if img.Caption != "" {
					captionChunk := &types.Chunk{
						...
						KnowledgeBaseID: knowledge.KnowledgeBaseID,
						Content:         img.Caption,
						ChunkIndex:      maxSeq + i*100 + 2, // Use non-conflicting index method
						IsEnabled:       true,
						CreatedAt:       time.Now(),
						UpdatedAt:       time.Now(),
						StartAt:         int(img.Start),
	...
	// Only set forward/backward relationships for text-type Chunks
	...
	// Set forward/backward relationships between text Chunks
...
func (s *knowledgeService) generateQuestionsWithContext(ctx context.Context,
	...
	if prevContent != "" || nextContent != "" {
		contextSection = "## Context Information (for reference only, to help understand main content)\n"
		if prevContent != "" {
			contextSection += fmt.Sprintf("[Previous Context] %s\n", prevContent)
		}
		if nextContent != "" {
			contextSection += fmt.Sprintf("[Next Context] %s\n", nextContent)
		}
		contextSection += "\n"
	}
...
- Questions must not use any pronouns or referential words (such as "it", "this", "the document", "this article", "in the text", "its", etc.), must use specific names instead
...
- Each question should be concise and clear, limited to 30 characters
...
## Output Format
Output the question list directly, one question per line, without numbers or other prefixes.`
...
func (s *knowledgeService) UpsertFAQEntries(ctx context.Context,
	...
	// Validate knowledge base exists and is valid
	kb, err := s.validateFAQKnowledgeBase(ctx, kbID)
	if err != nil {
		return "", err
	}
	...
	// Check if there is an ongoing import task (via Redis)
	runningTaskID, err := s.getRunningFAQImportTaskID(ctx, kbID)
	if err != nil {
		logger.Errorf(ctx, "Failed to check running import task: %v", err)
		// Check failure does not affect import, continue execution
	} else if runningTaskID != "" {
		logger.Warnf(ctx, "Import task already running for KB %s: %s", kbID, runningTaskID)
		return "", werrors.NewBadRequestError(fmt.Sprintf("This knowledge base already has an import task in progress (Task ID: %s), please wait for completion before trying again", runningTaskID))
	}
	...
	if err := s.setRunningFAQImportTaskID(ctx, kbID, taskID); err != nil {
		...
		// Does not affect task execution, continue
	}
...
// calculateAppendOperations calculates entries to process in Append mode, skipping existing entries with identical content
// Also filters out entries with duplicate standard questions or similar questions in the same batch or existing knowledge base
func (s *knowledgeService) calculateAppendOperations(ctx context.Context,
	...
	// 1. Query all existing FAQ chunks' metadata in the knowledge base
	...
	// 2. Build set of existing standard questions and similar questions
	...
	// 3. Build set of standard questions and similar questions in current batch (for batch deduplication)
	...
	for _, entry := range entries {
		...
		// Check if standard question is duplicate (with existing or same batch)
		...
		// Check if similar questions have duplicates (with existing or same batch)
		...
		// Add current entry's standard question and similar questions to batch set
...
func (s *knowledgeService) executeFAQImport(ctx context.Context, taskID string, kbID string,
	...
	for i := 0; i < remainingEntries; i += faqImportBatchSize {
		...
		if err := s.updateFAQImportProgressStatus(ctx, taskID, types.FAQImportStatusProcessing, progress, totalEntries, actualProcessed, fmt.Sprintf("Processing entry %d/%d", actualProcessed, totalEntries), ""); err != nil {
...
func (s *knowledgeService) GetFAQEntry(ctx context.Context,
	...
	if chunk.KnowledgeBaseID != kb.ID || chunk.TenantID != tenantID {
		return nil, werrors.NewNotFoundError("FAQ entry does not exist")
	}
...
func (s *knowledgeService) UpdateFAQEntry(ctx context.Context,
	...
	if chunk.KnowledgeBaseID != kb.ID {
		return werrors.NewForbiddenError("No permission to operate this FAQ entry")
	}
	if chunk.ChunkType != types.ChunkTypeFAQ {
		return werrors.NewBadRequestError("Only FAQ entries support update")
	}
	meta, err := sanitizeFAQEntryPayload(payload)
	if err != nil {
		return err
...
func (s *knowledgeService) UpdateKnowledgeTag(ctx context.Context, knowledgeID string, tagID *string) error {
	...
	if tagID != nil && *tagID != "" {
		...
		if tag.KnowledgeBaseID != knowledge.KnowledgeBaseID {
			return werrors.NewBadRequestError("Tag does not belong to current knowledge base")
		}
...
func (s *knowledgeService) UpdateKnowledgeTagBatch(ctx context.Context, updates map[string]*string) error {
	...
	for _, knowledge := range knowledgeList {
		...
		if tagID != nil && *tagID != "" {
			tag, ok := tagMap[*tagID]
			if !ok {
				return werrors.NewBadRequestError(fmt.Sprintf("Tag %s does not exist", *tagID))
			}
			if tag.KnowledgeBaseID != knowledge.KnowledgeBaseID {
...
func (s *knowledgeService) UpdateFAQEntryTag(ctx context.Context, kbID string, entryID string, tagID *string) error {
	...
	if chunk.KnowledgeBaseID != kb.ID || chunk.ChunkType != types.ChunkTypeFAQ {
		return werrors.NewBadRequestError("Only FAQ entry tags support update")
	}
...
func (s *knowledgeService) UpdateFAQEntryTagBatch(ctx context.Context, kbID string, updates map[string]*string) error {
	...
	if len(tagIDSet) > 0 {
		...
		for _, tagID := range tagIDs {
			...
			if tag.KnowledgeBaseID != kb.ID {
				return werrors.NewBadRequestError(fmt.Sprintf("Tag %s does not belong to current knowledge base", tagID))
			}
			tagMap[tagID] = tag
		}
	}
...
func (s *knowledgeService) DeleteFAQEntries(ctx context.Context,
	...
	if len(entryIDs) == 0 {
		return werrors.NewBadRequestError("Please select FAQ entries to delete")
	}
	kb, err := s.validateFAQKnowledgeBase(ctx, kbID)
	if err != nil {
		return err
...
// Bot Answers (required-multiple separated by ##), Answer All (optional-default FALSE), Disabled (optional-default FALSE),
// Forbid Recommendation (optional-default False, can be recommended)
...
func (s *knowledgeService) buildFAQCSV(chunks []*types.Chunk, tagMap map[string]string) []byte {
	...
	headers := []string{
		"Category (required)",
		"Question (required)",
		"Similar Questions (optional-multiple separated by ##)",
		"Negative Questions (optional-multiple separated by ##)",
		"Bot Answers (required-multiple separated by ##)",
		"Answer All (optional-default FALSE)",
	...
	for _, chunk := range chunks {
		...
		row := []string{
			...
			boolToCSV(!chunk.IsEnabled),                                 // Disabled: inverted
			boolToCSV(!chunk.Flags.HasFlag(types.ChunkFlagRecommended)), // Forbid recommendation: inverted
		}
...
func (s *knowledgeService) validateFAQKnowledgeBase(ctx context.Context, kbID string) (*types.KnowledgeBase, error) {
	...
	if kb.Type != types.KnowledgeBaseTypeFAQ {
		return nil, werrors.NewBadRequestError("Only FAQ knowledge base supports this operation")
	}
	return kb, nil
}
...
func buildFAQChunkContent(meta *types.FAQChunkMetadata, mode types.FAQIndexMode) string {
	...
	// Negative examples should not be included in Content, as they should not be indexed
	// Answers are included based on index mode
...
// checkFAQQuestionDuplicate checks if standard question and similar questions are duplicated with other entries in knowledge base
// excludeChunkID is used to exclude the currently editing entry (used when updating)
func (s *knowledgeService) checkFAQQuestionDuplicate(
	...
	// Query all existing FAQ chunks' metadata in knowledge base
	existingChunks, err := s.chunkRepo.ListAllFAQChunksWithMetadataByKnowledgeBaseID(ctx, tenantID, kbID)
	if err != nil {
		return fmt.Errorf("failed to list existing FAQ chunks: %w", err)
	}

	// Build set of existing standard questions and similar questions
	for _, chunk := range existingChunks {
		// Exclude currently editing entry
		...
		// Check if standard question is duplicate
		...
		// Check if current standard question duplicates existing similar questions
		for _, q := range existingMeta.SimilarQuestions {
			if q == meta.StandardQuestion {
				return werrors.NewBadRequestError(fmt.Sprintf("Standard question \"%s\" duplicates existing similar question", meta.StandardQuestion))
			}
		}

		// Check if current similar questions duplicate existing standard question
		for _, q := range meta.SimilarQuestions {
			if q == existingMeta.StandardQuestion {
				return werrors.NewBadRequestError(fmt.Sprintf("Similar question \"%s\" duplicates existing standard question", q))
			}
		}

		// Check if current similar questions duplicate existing similar questions
		for _, q := range meta.SimilarQuestions {
...
func (s *knowledgeService) resolveTagID(ctx context.Context, kbID string, payload *types.FAQEntryPayload) (string, error) {
	...
	if payload.TagName != "" && payload.TagName != "Uncategorized" {
...
func sanitizeFAQEntryPayload(payload *types.FAQEntryPayload) (*types.FAQChunkMetadata, error) {
	...
	if len(meta.Answers) == 0 {
		return nil, werrors.NewBadRequestError("At least one answer must be provided")
	}
	return meta, nil
}
...
func (s *knowledgeService) ProcessFAQImport(ctx context.Context, t *asynq.Task) error {
	...
	if err != nil {
		...
		// 如果是最后一次重试，更新状态为失败
		if isLastRetry {
			if updateErr := s.updateFAQImportProgressStatus(ctx, payload.TaskID, types.FAQImportStatusFailed, 0, len(payload.Entries), 0, "Failed to get knowledge base", err.Error()); updateErr != nil {
