...
func getEnrichedPassage(ctx context.Context, result *types.SearchResult) string {
	...
	if result.ImageInfo != "" {
		...
		if err != nil {
			...
			// Extract all image descriptions and OCR text
			for _, img := range imageInfos {
				if img.Caption != "" {
					enrichments = append(enrichments, fmt.Sprintf("Image Caption: %s", img.Caption))
				}
				if img.OCRText != "" {
	...
	// Combine content and enhanced information
	if combinedText != "" {
		combinedText += "\n\n"
	}
