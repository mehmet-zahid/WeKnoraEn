...
func (e *AgentEngine) streamReflectionToEventBus(
	...
	reflectionPrompt := fmt.Sprintf(`Please evaluate the result of calling tool %s just now, and decide the next action.
...
func (e *AgentEngine) streamFinalAnswerToEventBus(
	...
	finalPrompt := fmt.Sprintf(`Based on the above tool call results, please generate a complete answer for the user's question.

User question: %s

Requirements:
1. Answer based on actually retrieved content
2. Clearly label information sources (chunk_id, document name)
3. Organize answer in a structured way
4. If information is insufficient, state honestly
