...
func (t *DataAnalysisTool) formatQueryResults(results []map[string]string, query string) string {
	...
	if len(results) > 10 {
		output.WriteString(fmt.Sprintf("Displaying all %d records. It is recommended to use LIMIT clause to limit result count for better performance.\n\n", len(results)))
	}
...
func (t *TableSchema) Description() string {
	...
	builder.WriteString(fmt.Sprintf("Column Count: %d\n", len(t.Columns)))
	builder.WriteString(fmt.Sprintf("Row Count: %d\n\n", t.RowCount))
	builder.WriteString("Column Information:\n")
