...
func (t *GetDocumentInfoTool) Execute(ctx context.Context, args json.RawMessage) (*types.ToolResult, error) {
	...
	for _, knowledgeID := range knowledgeIDs {
		...
		go func(id string) {
			...
			if err != nil {
				mu.Lock()
				results[id] = &docInfo{
					err: fmt.Errorf("failed to get document information: %v", err),
				}
				mu.Unlock()
				return
			}
	...
	if len(successDocs) == 0 {
		return &types.ToolResult{
			Success: false,
			Error:   fmt.Sprintf("Failed to get any document information. Errors: %v", errors),
		}, fmt.Errorf("all document retrievals failed")
	}

	// Format output
	output := "=== Document Information ===\n\n"
	output += fmt.Sprintf("Successfully retrieved %d / %d document information\n\n", len(successDocs), len(knowledgeIDs))
